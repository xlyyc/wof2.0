/**
 * @widgetClass SubmitSpanner class
 * @package wof.functionWidget.spanner
 * @copyright author
 * @Time: 13-8-5 下午1:29
 */
wof.functionWidget.spanner.SubmitSpanner = function () {
    this._version = '1.0';

    //初始化构件元数据 包括名称、标题、发送的消息
    this._meta = {};
    this._meta.name = 'wof.functionWidget.Submit';
    this._meta.title = '提交';
    ...
    onReceiveMessage.push({id:'wof.functionWidget.Submit_active',method:'this._receivePropertysAndRenderSelf(message.sender);'});
...
wof.functionWidget.spanner.SubmitSpanner.prototype = {
    ...
    _receiveAndProcessParameters:function(parameters){
        if(parameters.id==this.getPropertys().id){
            ...
            node.setData(parameters);
            node.render();
            node.sendMessage("wof.functionWidget.Submit_active");
        }
    }
