...
if (!wis.util.Selector) {
    wis.util.Selector = {
            ...
            find: function(queryString) {
                var sls = new wis.util.SelectorList();
                var selectors = wis.util.Selector._parser(queryString);
                for(var i=0;i<selectors.length;i++){
                    var ss = selectors[i];
                    if(ss.type=='*'){
                        console.log('查找所有对象');
                    }else if(ss.type=='#'){
                        sls = wis.util.Selector._getObjectById(ss.query);
            ...
            _getObjectById: function(oid) {
                var sls = new wis.util.SelectorList();
                var obj = wis.util.ObjectManager.get(oid);
                sls.add(obj);
                return sls;
            }
